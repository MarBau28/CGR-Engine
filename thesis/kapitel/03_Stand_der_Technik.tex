...
native Kompatibilität mit hardwarebasiertem \ac{MSAA} sowie eine grundlegende Einschränkung in der Darstellung
transluzenter Oberflächen.
Da die Beleuchtung im Screen-Space berechnet wird, sind die für \ac{MSAA} erforderlichen Geometriedaten zu diesem
Zeitpunkt verworfen.
Stattdessen müssen Verfahren wie \ac{TAA} eingesetzt
...
um diese Metadaten im zweiten Schritt für globale, Screen-Space-basierte \ac{NPR}-Effekte nutzbar zu machen.
...
theoretischen Hürden in der Praxis umgangen werden.
...
\textit{Warp Divergence} (vgl.\ Kap.~\ref{subsec:020203_warp_divergence_and_register_pressure}).
...
Die Architektur nutzt hybrides Clustered Deferred und Forward Shading um komplexe
...
Restriktionen klassischer Forward- und Deferred-Pipelines, insbesondere im Kontext der selektiven Stilisierung
mittels \ac{NPR}.
...
Ziel ist die Erarbeitung einer architektonischen Basis für ein fundiertes empirisches Entscheidungsmodell.
...
Theoretische Performance-Engpässe, wie etwa Bandbreitenlimitierungen in \acs{G-Buffer}n oder Registerdruck in
...
dieses Vorhabens (siehe Abb.~\ref{fig:hydra_example}).
Durch die Erhebung granularer Rohdaten unter skalierender Stil-Entropie und Szenenkomplexität soll das in
Kapitel~\ref{sec:0102_goal_and_core_question} skizzierte heuristische
...
\begin{figure}[htbp]
    ...
    \caption[Screenshot aus dem HyDra-Framework]{Screenshot aus HyDra-Framework.}
