...
% Evaluationsziel
Wie in Kapitel~\ref{sec:0202_rendering_paradigms} dargelegt, weisen die theoretischen Architekturparadigmen des
Forward-, Deferred-Uber- und Deferred-Volume-Renderings spezifische Leistungsmerkmale auf.
...
Dadurch soll vermieden werden, eine Pipeline gegenüber den anderen als universell leistungsfähiger darzustellen.

% Technische Hypothesen
Auf Basis der in Kapitel~\ref{ch:02_grundlagen} dargelegten theoretischen Faktenlage werden für die implementierten
Rendering-Pipelines spezifische Leistungshypothesen formuliert.
...
\begin{itemize}
    \item \hypertarget{hyp:H1.1}{\textbf{Hypothese H1.1 (Forward GPU-Engpass):}}
    ...
    Beleuchtungsberechnung von $\mathcal{O}(\text{Fragmente} \cdot \text{Lichter})$ (vgl.~\ref{eq:forward_shading_complexity}) zu
    signifikanten Leistungseinbußen führt.

    \item \hypertarget{hyp:H1.2}{\textbf{Hypothese H1.2 (Forward CPU-Engpass):}}
    Hinsichtlich der \ac{CPU}-Auslastung wird angenommen, dass das \ac{CPU}-seitige Filtern und Zuordnen von
    Lichtquellen pro Objekt sowie das hochfrequente Binden objektspezifischer Licht-Uniforms einen signifikanten
    \ac{API}-Overhead provoziert, der die \ac{CPU}-Zuweisungszeit bei anwachsender Instanzenanzahl nicht-linear
    skalieren lässt~\parencite[vgl.][Abschn.~1]{haradaForwardBringingDeferred2012}.

    \item \hypertarget{hyp:H2}{\textbf{Hypothese H2 (Deferred-Uber Style-Entropie):}}
    Für die Deferred-Pipeline mit einfachem Beleuchtungs-Pass (Single-Pass) und monolithischem Uber-Shader wird anhand
    von Kapitel~\ref{subsec:020202_deferred_rendering} und~\ref{subsec:020303_warp_divergence_and_register_pressure}
    prognostiziert, dass der signifikanteste Leistungsengpass bei einer hohen räumlichen Stil-Entropie auftritt.
    Wie in Kapitel~\ref{subsec:020303_warp_divergence_and_register_pressure} theoretisch hergeleitet, erzwingen
    feingranular gestreute \ac{NPR}-Effekte divergente Code-Zweige (Warp-Divergenz und Registerdruck).
    Diese kombinierten Hardware-Flaschenhälse zwingen die Architektur zu einer sequenziellen Abarbeitung der Pixel
    und lassen eine messbare Degradation der Shading-Performance bei stark vermischten Effekten erwarten.

    \item \hypertarget{hyp:H3}{\textbf{Hypothese H3 (Deferred-Volume Speicherbandbreite):}}
    Für die Deferred-Pipeline auf Basis von Light-Volumes (Deferred-Volume) besagt Hypothese H3
...
eingesetzt werden, um Mikrometriken wie exakte Cache-Hit-Raten oder Thread-Maskierungen auf Hardware-Ebene
auszulesen, werden die architektonischen Engpässe deduktiv aus den gemessenen Rendering-Laufzeiten abgeleitet.
...
Die referenzierten detaillierten Tabellen sind hierzu sind unter Anhang~\ref{ch:AA_configuration_and_variable_mapping}
zu finden.
...
Leistungsgrenzen des Versuchssystems.
...
Nach der Definition der Hardware- und Software-Spezifikationen erfordert das Evaluations-Design eine exakte Abgrenzung
der unabhängigen Variablen.
Das Framework variiert diese Parameter während der Testreihen systematisch, um gezielt isolierte
Hardware-Leistungsgrenzen zu provozieren.
...
% Aufwärmphase.
Vor der aktiven Datenaufzeichnung durchläuft das System eine Warm-up-Phase von zehn Sekunden.
Dieser Prozess dient dazu, die Hardware-Caches und Textur-Caches
aufzubauen~\parencites(vgl.)()[Kap.~18.4, Abschn.~\textit{Memory Issues}]{akenine-mollerRealtimeRendering2019}
...
für die Messung minimiert.
Während der Wechsel zwischen Rendering-Pipelines oder beim Voranschreiten abhängiger Schritt-Variablen wird ebenfalls
eine Warm-up-Pause von zehn Sekunden erzwungen.
...
dedizierten Arrays für den Export.

% Datenaufbereitung.
Die präsentierten Graphen und Messergebnisse basieren auf arithmetischen Mittelwerten der Sampling-Phase.
Zur Bereinigung unvorhersehbarer Micro-Stutterer (\textit{Frame-Rate Spikes}) kommt das etablierte
$3\sigma$-Kriterium der Statistik zum Einsatz~\parencite[vgl.][]{pukelsheimThreeSigmaRule1994}.
Aus dem initialen Datensatz $X$ einer Parameter-Konfiguration werden der empirische Mittelwert $\mu$ sowie die
Standardabweichung $\sigma$ berechnet.
Ein Messwert $x_i \in X$ wird ausschließlich einbezogen, sofern er innerhalb des $3\sigma$-Intervalls liegt:

\begin{equation}
    \mu - 3\sigma \le x_i \le \mu + 3\sigma
\end{equation}

Werte außerhalb dieses Intervalls gelten als statistische Ausreißer und werden gefiltert.
Diese Ausreißerbereinigung stellt sicher, dass die finalen Messdaten das rein architektonische und algorithmische
Skalierungsverhalten der Rendering-Pipelines isoliert abbilden.
...
erzeugen~\parencite[vgl.][S.~55]{burnsVisibilityBufferCacheFriendly2013}.
...
verschiedenen Rendering-Pipelines nahezu identisch und skalieren auch bei erhöhtem Detailgrad (\ac{LOD})
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.95\linewidth]{abbildungen/5-2-1_bottleneck_divergence}
...
Bandbreitenkosten für reine Geometrieszenarien~\parencite[vgl.][S.~905]{akenine-mollerRealtimeRendering2019}.
...
unterschreitet~\parencite[vgl.][S.~1]{fatahalianReducingShadingGPUs2010}.
...
würden~\parencites(vgl.)()[Kap.~18.2.3]{akenine-mollerRealtimeRendering2019}[Folie~\textit{Tiny triangles}]{Karis_Nanite_SIGGRAPH_Advances_2021_final}.

\subsection{{Objektanzahl}}
\label{subsec:050202_obejct_count_eval}

% Einleitung.
Das Ziel dieser Testreihe besteht in der isolierten Evaluation der \ac{CPU}-Performance beim Dispatch-Prozess sowie
bei der Szenenlogik (Frustum Culling, Sichtbarkeitsprüfung und Matrix-Updates).
Um jegliche \ac{GPU}-Engpässe auszuschließen, wird die Geometrie-In\-stan\-zen\-an\-zahl ohne Beleuchtungseinflüsse
($N_{\text{Light}} = 0$) und bei minimalem Detailgrad ($LOD = 0$, 12-Dreieck-Würfel) skaliert.
Hierbei wird geprüft, ob der \ac{CPU}-Overhead die Skalierung limitiert und welche architektonischen Unterschiede
zwischen den Rendering-Pipelines auf Host-Seite (\ac{CPU}) bestehen.
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.95\linewidth]{abbildungen/5_2_2_cpu_scaling_logic_vs_render}
...
Lichtliste evaluieren und binden muss~\parencite[vgl.][S.~891]{akenine-mollerRealtimeRendering2019}.
...
Das schlechtere Skalierungsverhalten der Forward-Pipeline bei Verarbeitung des logischen Szenen-Passes auf der \ac{CPU}
verifiziert somit \hyperlink{hyp:H1.2}{Hypothese H1.2}.

% Limitationen.
Diese Konzeption stellt eine bewusste Systementscheidung des Frameworks dar.
Zwar hätte ein alternativer Multi-Pass-Forward-Ansatz (ein Render-Pass pro Lichtquelle) diese objektspezifische
Suchlogik umgangen~\parencite[vgl.][S.~1]{Thaler2011DeferredRendering}, jedoch wurde dieses Paradigma im Rahmen des
Untersuchungsrahmens gezielt ausgeschlossen.

\subsection{{Overdraw-Dichte}}
\label{subsec:050203_overdraw_eval}

% Einleitung.
Der folgende Versuch evaluiert den Einfluss extremer Z-Achsen-Stapelung (Overdraw) auf die Speicherbandbreite der
\ac{GPU}.
Durch das Skalieren von $N_{\text{Inst}}$ auf engstem Raum ($r_{\text{Spawn}} = 30{,}0$) wird geprüft, ob die höhere
\ac{VRAM}-Bandbreite der Deferred-Pipelines beim Beschreiben des \ac{G-Buffer}s zu Bandbreiten-Sättigungen führt.

% Definition Overdraw.
Der theoretische Overdraw-Faktor $O_{\text{Draw}}$ beschreibt das Verhältnis der summierten, projizierten
Objektflächen zur Gesamtauflösung des Bildschirms:

\begin{equation}
    O_{\text{Draw}} = \frac{\sum A_{\text{Objekt}}}{A_{\text{Bildschirm}}}
\end{equation}

Dieser Kennwert gibt die durchschnittliche Anzahl an Geometrieschichten an, die theoretisch auf einem einzelnen
Pixel liegen.
Ein Wert von $O_{\text{Draw}} = 4{,}5$ bedeutet folglich theoretisch, dass die \ac{GPU} ein Pixel im Schnitt 4,5-mal
beschreiben müsste.
...
% Beschreibung.
Obwohl alle analysierten Rendering-Pipelines einen messbaren Anstieg der \ac{GPU}-Aus\-füh\-rungs\-zeit aufweisen, sind, wie
in Abbildung~\ref{fig:5_2_3_overdraw_fillrate_scaling} zu beobachten, die Abweichungen untereinander minimal.

% Ergebnis Graph
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.95\linewidth]{abbildungen/5_2_3_overdraw_fillrate_scaling}
...
auf Hardware-Ebene verworfen werden, bevor kostspielige Schreiboperationen in den Framebuffer
stattfinden~\parencite[vgl.][S. 801]{akenine-mollerRealtimeRendering2019}.
...
bandbreitenspezifischen Limitierungen über anwachsende Render-Target-Auflösungen auszuloten.
Im Rahmen der Analyse werden die \ac{API} und die Speicherkosten des \ac{G-Buffer}s sowie der zunehmende
\ac{ALU}-Aufwand des Forward-Renderings betrachtet.
...
Pixelanzahl gekoppelt sind~\parencite[vgl.][Abschn.~9.2]{Chapter9Deferred}.
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.95\linewidth]{abbildungen/5_3_1_resolution_scaling_crossover}
...
% Deduktion.
Die starken Einbußen von Forward- gegenüber den Deferred-Pipelines werden im Zusammenhang mit \textit{Shading-Overdraw}
vermutet.
...
~\parencite[vgl.][Abschn.~1]{mallettDeferredAdaptiveCompute2018}.
...
% Beobachtungen.
Bei der vergleichenden Betrachtung der Messergebnisse (siehe Abb.~\ref{fig:5_3_2_1_base_bandwidth_tax_means} und
Abb.~\ref{fig:5_3_2_2_base_bandwidth_tax_means}) lassen sich zwei zentrale Effekte identifizieren:

\begin{itemize}
    \item \textbf{Verhalten bei 1080p-Auflösung:} Bei der Initialisierung des bildschirmfüllenden Boden-Quads skalierten
    alle untersuchten Pipelines annähernd identisch mit einem $t_{\text{GPU}}$-Anstieg von jeweils rund
    \qty{64}{\percent}.
    Die Deferred-Pipelines weisen dabei eine geringfügig höhere Basislast auf als die Forward-Referenz.
    \item \textbf{Skalierungsdivergenz bei 4K-Auflösung:} Bei einer Erhöhung der Render-Target-Auflösung auf 4K zeigt
    die Deferred-Volume-Pipeline eine signifikant schlechtere Skalierung im Vergleich zu den übrigen
    Single-Pass-Architekturen.
\end{itemize}

% Ergebnis Graph 1
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.95\linewidth]{abbildungen/5-3-2-1_BaseBandwidthTax_Means}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.95\linewidth]{abbildungen/5-3-2-2_BaseBandwidthTax_Means}
...
% Deduktion.
Der Vergleich der Messreihen zwischen 1080p und 4K deutet auf einen hardwareseitigen Bandbreiten-Flaschenhals hin.
Bei einer 1080p-Auflösung reicht die \ac{VRAM}-Bandbreite noch aus, um den Datenstrom von rund zwei Millionen Pixeln
effizient zu verarbeiten.
Die vervierfachte Datenlast der 4K-Auflösung führt allerdings zur Sättigung des Speicherbusses.
Läge die Ursache stattdessen in einer reinen Auslastung der Rechenkerne (\ac{ALU}), müssten alle Pipelines unter 4K
ein proportional ähnliches Skalierungsverhalten zeigen.
...
Blending zur Akkumulation der Lichtbeiträge~\parencite[vgl.][S.~9]{lauritzenDeferredRenderingCurrent}.
...
Wie in Kapitel~\ref{subsec:040201_forward_pipeline} beschrieben, sind sowohl die Forward-Pipeline
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.95\linewidth]{abbildungen/5-4-1_LightCount_Means}
...
Allgemeinen~\parencite[vgl.][S.~1]{Thaler2011DeferredRendering}.
...
Die höhere Effizienz des Deferred-Volume-Ansatzes lässt sich auf die räumliche Eingrenzung des
Evaluationsbereichs einzelner Light-Volumes zurückführen.
...
tatsächlich beeinflusst~\parencite[vgl.][S.~886]{akenine-mollerRealtimeRendering2019}.
...
Diese Beobachtung verifiziert \hyperlink{hyp:H1.1}{Hypothese H1.1}: Aus der multiplikativen Komplexität der
direkten Beleuchtungsberechnung entsteht bei der Forward-Architektur mit steigender Lichtkomplexität unweigerlich ein
Performance-Defizit.
...
Abbildung~\ref{fig:5_4_2_2_light_singularity_means} verdeutlicht diesen Effekt nochmals gezielt.
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.95\linewidth]{abbildungen/5-4-2-1_LightIntensity_Means}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.95\linewidth]{abbildungen/5-4-2-2_LightSingularity_Means}
...
auszuführen~\parencites(vgl.)()[Folie~9]{lauritzenDeferredRenderingCurrent}[S.~676]{gregoryGameEngineArchitecture2018}
(vgl.\ Kap.~\ref{subsec:020302_memory_bandwidth}).
...
% Einleitung.
In Unterkapitel~\ref{subsec:050203_overdraw_eval} wurde der Einfluss des Overdraws bereits für reine Geometrie
evaluiert.
Analog dazu untersucht dieser Versuchsaufbau das Verhalten unter zusätzlicher Beleuchtungslast.
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.95\linewidth]{abbildungen/5_4_3_decoupling_proof}
...
darstellt~\parencites(vgl.)()[Abschn.~1]{Thaler2011DeferredRendering}.
Die Ergebnisse dieser Evaluation bedingen somit eine Relativierung der \hyperlink{hyp:H1.1}{Hypothese H1.1}:
...
verdeckter Fragmente vermieden~\parencite[vgl.][S.~801~u.~803]{akenine-mollerRealtimeRendering2019}
(vgl.\ Kap.~\ref{subsec:020301_overdraw}).
...
% Einleitung.
In den nachfolgenden, zusammengefassten Versuchen werden die Einflüsse von stilistischer Komplexität durch die Anwendung
diverser \ac{NPR}-Stile analysiert.
Neben dem bisher standardmäßig verwendeten Blinn-Phong-Shading finden auch Toon-Shading und Gooch-Shading Anwendung
(vgl.\ Kap.~\ref{sec:0403_npr_payloads}), um eine variable Bandbreite algorithmischer und stilistischer Ausprägungen
abzudecken.
...
In dem vorliegenden Versuch \textit{A} wird eine sortierte (niedrige Entropie) mit einer gestreuten (hohe
Entropie) Objektverteilung verglichen, wobei die Anzahl der Stile konstant bei drei bleibt.
In dem weiteren Versuch \textit{B} wird die Anzahl der simultan aktiven Shading-Stile von 1 bis 3 (Blinn, Gooch,
Toon) skaliert.
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.95\linewidth]{abbildungen/5_5_1_entropy_divergence_means}
...
\begin{figure}[htbp]
    ...
    \begin{subfigure}[b]{0.49\textwidth}
        \centering
        \includegraphics[width=0.95\linewidth]{abbildungen/hydra_capture_20260628_110904}
    ...
    \begin{subfigure}[b]{0.49\textwidth}
        \centering
        \includegraphics[width=0.95\linewidth]{abbildungen/hydra_capture_20260628_110909}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.95\linewidth]{abbildungen/5_5_2_style_combinatorics_gpu}
...
Manifestation der \textit{Warp Divergence} dar und verifiziert \hyperlink{hyp:H2}{Hypothese H2} eindeutig.
...
Warps~\parencite[vgl.][S.~886]{akenine-mollerRealtimeRendering2019}.
...
Ohne den Einsatz dedizierter Hardware-Profiler lässt sich nicht zweifelsfrei ermitteln, inwieweit ein erhöhter
...
umsetzbar sind (vgl.\ Kap.~\ref{subsec:040201_forward_pipeline}) und ausschließlich auf gesamter Screen-Space-Ebene
wirken, lässt sich keine Zielbild-Parität erreichen.
...
Bei einem Radius von 16 ist Deferred-Uber um $\approx$ \qty{2,5}{\milli\second} schneller als
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.95\linewidth]{abbildungen/5_5_3_kernel_bandwidth_scalability}
...
Die Deferred-Uber-Pipeline operiert mittels \textit{Pre-Lit}-Kuwahara-Filter, um einen algorithmischen Kollaps durch
redundante und somit ineffiziente Beleuchtungsberechnungen pro Nachbarpixel zu verhindern.
...
% Kapiteleinleitung.
In der finalen Testreihe werden alle übergeordneten Einflüsse aus den vorangegangenen Benchmarks in ein kombiniertes
Szenario integriert, um das Zusammenwirken der bislang isoliert untersuchten Effekte in ihrer Gesamtheit zu
analysieren.
Die nachfolgenden Evaluationen werden in einem Schritt zusammengefasst, bestehen im Detail aus drei verschiedenen
...
eine Zusatzlast von \qty{0,10}{\milli\second} erfährt (vgl.\ Kap.~\ref{subsec:050501_entropy_eval}).
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.95\linewidth]{abbildungen/5_6_cumulative_rendering_cost_combined}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.95\linewidth]{abbildungen/5_6_flythrough_variance_npr}
...
(vgl.\ Ergebnisse aus Kap.~\ref{sec:0502_geometry_eval}).
...
(vgl.\ Ergebnisse aus Kap.~\ref{subsec:050401_light_count_eval}).
...
divergierende \ac{NPR}-Code-Zweige erzwungen wird (vgl.\ Ergebnisse aus Kap.~\ref{subsec:050501_entropy_eval}).
...
(vgl.\ Ergebnisse aus Kap.~\ref{subsec:050402_light_intensity_and_overlap_eval}).
