...
Leistungsmessungskomponenten (Benchmark-Orchestrierung) erläutert.
...
Für die Deferred-Pipelines wird zudem der Semantische \ac{G-Buffer} aufbereitet, wofür im Rahmen der Szeneverwaltung
...
(vgl.\ Kapitel~\ref{subsec:040402_async_hardware_telemetry}).
...
Dieses verwaltet den Aufbau der Geometrie, insbesondere die Punktlicht-Proxies und die beleuchteten Geometrien.
...
innerhalb des \ac{HyDra}-Frameworks in der kreisförmigen Objektwolke mit anteiliger Verteilung der Stile.
...
(vgl.\ Kapitel~\ref{subsec:020102_deferred_rendering}).
...
wie naive Forward-Ansätze, allerdings nur selten Anwendung (vgl.\ Kapitel~\ref{sec:0301_academic_state}).
...
Punktlichtquellen gekennzeichnet (vgl.\ Kapitel~\ref{eq:ubo-limits}).
...
(vgl.\ Abbildung~\ref{fig:040202_pre-post-lit_comparison}).
...
\begin{longtable}{@{} p{0.2\textwidth} p{0.4\textwidth} p{0.35\textwidth} @{}}
    \caption{Ausführung der verwendeten Shading-Effekte und NPR-Stile in HyDra-Framework.}
    ...
    (siehe Abbildung~\ref{fig:040202_pre-post-lit_comparison}).\\
    \addlinespace
    ...
    klassischer Cel-Animationen (siehe Abbildung~\ref{fig:040201_outline_comparison}). \\
    \addlinespace
    ...
    technische Illustrationen (siehe Abbildung~\ref{fig:gooch_example}). \\
    \addlinespace
    ...
    Bildraum (siehe Abbildung~\ref{fig:hydra_sobel-outlines}). \\
    \addlinespace
    ...
    Rückseiten (siehe Abbildung~\ref{fig:hydra_inverted-hull}). \\
\end{longtable}
...
(siehe Abbildung~\ref{fig:npr_spatial_low_pixel_problem}).
...
struct EngineState {
    ...
    bool requestScreenshot = false;
        ...
        \end{minted}
    \end{codeframe}
    \caption{C++ Code des \texttt{EngineState}. Fungiert als Verwaltungselement des zentralen Engine-Zustands innerhalb
    des HyDra-Frameworks.}
...
% Einführung.
Für die Makro-Benchmark-Synthese (siehe Kapitel~\ref{sec:0506_macro_eval}) wird im Gegensatz zu den Evaluationen mit
statischen Blickwinkeln und unbewegter Geometrie eine Kamerafahrt durch die Szene in mehreren Durchgängen verwendet.
...
beginnen würde.
Eine Framezeit gekoppelte Kameraführung hätte einen gleichen Effekt.
...
% Frame-Locking.
Vor diesem Hintergrund erzwingt das \ac{HyDra}-Framework absoluten Determinismus während für diese Bewegungssequenzen
...
\begin{figure}[htbp]
    ...
    \resizebox{0.95\textwidth}{!}{%
        \begin{tikzpicture}[
            ...
            \node (q_start) [process, below=of state] {GPU-Profiler: \texttt{Begin()}\\(Start-Query)};
            \node (render) [process, below=of q_start] {Rendering-Pipeline\\(Forward / Uber / Volume)};
            \node (q_end) [process, below=of render] {GPU-Profiler: \texttt{End()}\\(End-Query)};
    ...
    \caption{UML-Aktivitätsdiagramm der Architektur des Benchmark-Lebenszyklus in HyDra-Framework.
