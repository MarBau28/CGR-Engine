...
\begin{figure}[htbp]
    ...
    \begin{tikzpicture}[
        ...
        decision/.style={draw, diamond, aspect=2, inner sep=2pt, text width=2.5cm, align=center},
        ...
        \node (dec_empty) [decision, below=of enqueue] {\texttt{pendingFrames}\\leer?};
        \node (end) [startstop, left=of dec_empty, xshift=-0cm] {Frame abschließen};
        \node (dec_full) [decision, below=of dec_empty] {\texttt{pendingFrames}\\voll?};
        \node (block_wait) [process, right=of dec_full, xshift=0.5cm] {\texttt{forceWait = true}\\CPU blockiert
            (Stall)};
        \node (dec_ready) [decision, below=of dec_full] {GPU-Daten\\verfügbar?};
        \node (extract) [process, right=of dec_ready, xshift=0.5cm] {Datenextraktion\\CSV-Export \& \texttt{pop()}};
        ...
        % Async Break
        \draw[->] (dec_ready) -| node[above left, font=\footnotesize] {Nein (\texttt{break})} (end.south);

    \end{tikzpicture}
    \caption{Architektur des Benchmark-Lebenszyklus. Der \textit{Control Flow} differenziert zwischen statischen Messungen und Frame-Locking (Suite 5.6) sowie zwischen asynchroner Telemetrie-Extraktion und forcierten CPU-Stalls bei Überlauf des Ring-Buffers.}
