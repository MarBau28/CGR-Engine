...
Determinismus.
Das Programm wird so strukturiert, dass die drei zu untersuchenden Rendering-Pipelines unter identischen
...
andererseits das Laden, Kompilieren und Verlinken aller Shader-Programme inklusive des \textit{Uniform-Mappings}.
...
einer zylinder-Förmigen Objektwolke dargestellt {siehe~\ref{fig:hydra_example}}.
...
Anhand dieser Indizes werden für jedes Objekt drei Arrays für die Style-ID, die räumliche
Position und die bereits vorab berechnete Transformationsmatrix, nach dem
\ac{SoA}-Muster~\cite[S.~793]{akenine-mollerRealtimeRendering2019}, gespeichert.
...
innerhalb des \ac{HyDra}-Frameworks in der zylindrischen Objektwolke mit anteiliger Verteilung der Stile.
...
if (state.useClusteredStyles) {
    ...
    styleId = activeStyles[i % static_cast<int>(numStyles)];
        ...
        \end{minted}
    \end{codeframe}
    \caption[C++ Code zur Berechnung der Style-ID-Verteilung in HyDra-Framework]{C++ Code zur Berechnung der \texttt{styleId}-Verteilung basierend auf dem Cluster-Status in
    HyDra-Framework.}
...
% G-buffer Layout.
Wie in Tabelle~\ref{tab:gbuffer_layout} aufgezeigt wird für das Render-Target \texttt{\ac{RT}0} (Albedo) ein 8-Bit-Format
(\texttt{R8G8B8A8}) genutzt.
Für den Normalen-Buffer (\texttt{\ac{RT}1}) wird hingegen ein 16-Bit-Format (\texttt{R16G16B16A16}) verwendet.
...
% Stilisierung.
Wie in Kapitel~\ref{subsec:040401_scene_and_state_parity} dargelegt, erfolgt die selektive Stilisierung in der
...
\begin{figure}[htbp]
    ...
    \end{subfigure}
    \caption[Toon-Shading und Kontureffekt-Vergleich in HyDra-Framework]{Toon-Shading und Vergleich von Kontureffekten in Forward- und Deferred-Pipelines mittels Inverted-Hull- und
...
if (useBlinnFallback) {
    // ...
        ...
        \end{minted}
    \end{codeframe}
    \caption[GLSL-Code des Deferred-Uber Beleuchtungspasses in HyDra-Framework]
    {GLSL-Code des Deferred-Uber Beleuchtungspasses in HyDra-Framework}
...
eines definierten Radius benötigt (Filter-Kernel~\cite[Abschn.~2]{kyprianidisImageVideoAbstraction2009}), müsste für eine \textit{Post-Lit}-Variante (Filterung des fertig beleuchteten Bildes)
...
von $\mathcal{O}(\text{Filter-Samples} \times \text{Lichtanzahl})$ pro Pixel, was eine Echtzeit-Ausführung ausschließt.
...
% Rechtes Bild
    ...
    {Visueller Vergleich von Pre-Lit und Post-Lit Kuwahara in HyDra-Framework.}
...
Zu diesem Zweck wird die \textit{Deferred-Volume-Pipeline} implementiert.
...
wird, durch das additive Blending der Light-Volumes, im Framebuffer(\acp{ROP}) und der \ac{VRAM}-Bandbreite
verortet~\cites[Folie~9]{lauritzenDeferredRenderingCurrent}[S.~676]{gregoryGameEngineArchitecture2018}.
...
Kompensation dieses Präzisionsverlusts und Vermeidung von Artefakten wie etwa Farbsättigung durch Clamping
angewiesen.
...
konkret das simultane Abfragen von G-Buffer-Identitätsdaten und dem akkumulierten Lichtpuffer.
...
\begin{longtable}{@{} p{0.2\textwidth} p{0.4\textwidth} p{0.35\textwidth} @{}}
    \caption[Shading-Effekte und NPR-Stile in HyDra-Framework]{Ausführung der verwendeten Shading-Effekte und NPR-Stile in HyDra-Framework.}
...
\begin{figure}[htbp]
    ...
    \caption[Gooch-Shading in HyDra-Framework]{Gooch-Shading in HyDra-Framework.}
...
(vgl.\ Listing~\ref{lst:kuwahara_optimization}).
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.95\linewidth]{abbildungen/hydra_capture_20260524_103923}
    \caption[Kollabieren der selektiven Kuwahara-Bildraumfilterung in HyDra-Framework]
    {Kollabieren der selektiven Kuwahara-Bildraumfilterung bei weit entfernten und kleinen Geometrien in
...
(vgl.\ Listing~\ref{lst:Engine_State}).
...
(wie Instanzenanzahl, Detailgrad, Filter-Radien etc.) exakt einmal auf den globalen Zustand an und variiert im
Testverlauf lediglich die untersuchte unabhängige Variable (vgl.\ Tab.~\ref{tab:independent_vars}).
Wie in Kapitel~\ref{subsec:040401_scene_and_state_parity} dargelegt nutzen alle untersuchten Rendering-Pipelines über die
...
struct EngineState {
    ...
    bool requestScreenshot = false;
        ...
        \end{minted}
    \end{codeframe}
    \caption[C++-Code des EngineState in HyDra-Framework]
    {C++ Code des \texttt{EngineState}. Fungiert als Verwaltungselement des zentralen Engine-Zustands innerhalb
    des HyDra-Frameworks.}
...
(vgl.\ Listing~\ref{lst:orchestrator_stall}).
...
if (allQueriesReady || emergencyStallRequired) {
        ...
        \end{minted}
    \end{codeframe}
    \caption[C++-Code zum Notfall-Stall-Verfahren in HyDra-Framework]
    {C++ Code zu Ring-Buffer und Notfall-Stall Verfahren während der Benchmark-Orchestrierung in
    HyDra-Framework}
...
\begin{figure}[htbp]
    ...
    \resizebox{0.95\textwidth}{!}{%
        \begin{tikzpicture}[
            ...
            \node (state) [process, below=of dec_macro] {Zustandsmutation\\(LOD, Geometrie, Lights)};
    ...
    {UML-Aktivitätsdiagramm der Architektur des Benchmark-Lebenszyklus in HyDra-Framework.
    Der Control-Flow differenziert zwischen statischen Messungen und Frame-Locking (Suite~\ref{sec:0506_macro_eval})
