...
% Einleitung
Wie in Kapitel 3.1 und 3.2 beschrieben, bietet sowohl die Forschung als auch die Industriepraxis diverse, sehr
spezialisierte Lösungen für architekturbedingte Restriktionen klassischer Forward- und Deferred-Pipelines, besonders auf
der Ebene der selektiven Stilisierungen im Bereich des Non-Photorealistic Renderings (NPR).
Da eine vollumfängliche Analyse aller hochspezifischen Industrie-Workarounds den Rahmen der praktischen Machbarkeit
sprengen würde, verortet sich die Forschungslücke der vorliegenden Arbeit stattdessen in der empirischen Rückbesinnung
auf die architektonischen Fundamente.
Um die systematische Verschiebung der Performance-Engpässe zu evaluieren, sollen primäre Rendering-Paradigmen,
namentlich naives Forward Rendering, klassisches Deferred Shading mit einfachen G-Buffer-Packing (via Style-IDs
und Uber-Shader) sowie Deferred Light Volumes als Referenz für Multi-Pass Architekturen, exemplarisch gegeneinander
antreten.
Das Ziel ist die Schaffung einer empirischen Basis für eine semi-holistische Entscheidungsmatrix, deren exakte
wissenschaftliche Abgrenzung im Folgenden dargelegt wird.

% Die spezifische Forschungslücke (Granularität vs. Globales Post-Processing)
Globale Screen-Space Filter sin in der Praxis weit verbreiteter Standard und performancetechnisch weitestgehend gelöst.
Im speziellen Falle der selektiven, objektbasierten Stilisierung, besonderen im Bereich der weitreichend eingesetzten
Deferred-Rendering-Paradigmen, kommen gängige Praktiken und Workarounds, je nach Zielbild, allerdings immer wieder an
grundlegende Grenzen.
Industrie-Workarounds wie Stencil-Masks, G-Buffer Packing, oder die Anwendung von hybriden Pipelines versprechen zwar
Abhilfe, sind jedoch aufgrund ihrer sehr individualisierten Anwendung nur wenig bis gar nicht hinsichtlich der ihnen
zugrundeliegenden Skalierungsmustern dokumentiert.
Wann erfordert die zugrundeliegende Scene welche Lösungsansätze und wo sind die grundlegenden Flaschenhälse.

% Der Mangel an empirischer Evidenz (Das Messproblem)
Zwar sind die theoretischen Bottlenecks (Bandbreitenlimits in G-Buffern, Register Pressure bei Uber-Shadern etc.)
bekannt und umfassend dokumentiert.
Wie Akenine-Möller et al. anmerken, verschieben sich diese Flaschenhälse in der Praxis jedoch hochdynamisch in
Abhängigkeit der Szenenkomplexität.
Es fehlt in der aktuellen Literatur eine isolierte, vergleichende und empirische Systemanalyse, welche die Limitationen,
im Speziellen für den Bereich NPR, quantifiziert.

% Lösungsansatz und Legitimation der vorliegenden Arbeit
Um diese identifizierte Forschungslücke zu schließen, bedarf es eines isolierten Messinstruments, das frei von
Engine-spezifischen Optimierungen die reinen Basis-Paradigmen der zugrundeliegenden Rendering-Architekturen evaluiert.
Zu diesem Zweck wird im Rahmen der vorliegenden Arbeit das Evaluierungs-Framework \textit{HyDra} konzipiert und
implementiert.
Durch die Erhebung granularer Rohdaten unter skalierender Stil-Entropie uns Szenekomplexität soll eine fundierte,
semi-holistische Entscheidungsmatrix abgeleitet werden.
