...
Beleuchtungsberechnung von $\mathcal{O}(\text{Fragmente} \cdot \text{Lichter})$ zu signifikanten Leistungseinbußen führt
(\hypertarget{hyp:H1.1}{\textbf{Hypothese H1.1}}).
...
(\hypertarget{hyp:H1.2}{\textbf{Hypothese H1.2}}).
...
(\hypertarget{hyp:H2}{\textbf{Hypothese H2}}).
...
Für die Deferred-Pipeline auf Basis von Light-Volumes postuliert \hypertarget{hyp:H3}{\textbf{Hypothese H3}}, dass diese
Architektur eine hohe Resilienz gegenüber räumlicher Stil-Entropie aufweist und die Branching-Problematik des Uber-Shaders umgeht.
...
eingesetzt wird, um Mikrometriken wie exakte Cache-Hit-Raten oder Thread-Maskierungen auf Siliziumebene auszulesen,
...
aufzubauen~\cites[Kap.~18.4, Abschn.~\textit{Memory Issues}]{akenine-mollerRealtimeRendering2019}[]{NVIDIATuringArchitectureWhitepaper}.
...
Betrachtung der Host-Seite zu gewährleisten.
...
\begin{figure}[htbp]
    ...
    \caption[Objektanzahl-Limitierung im CPU-Dispatch]{Messung von Logik-Laufzeit ($t_{\text{Logic}}$) und
...
Das schlechtere Skalierungsverhalten der Forward-Pipeline innerhalb der Traversierung des logischen
Szenen-Passes verifiziert weiterhin \hyperlink{hyp:H1.2}{Hypothese H1.2}.
...
Schreiboperationen in den Framebuffer oder G-Buffer stattfinden~\cite[S. 801]{akenine-mollerRealtimeRendering2019}.
...
Bei 4K werden Forward-Renderzeiten von nahezu \qty{17}{\milli\second} erreicht, während die Deferred-Pipelines mit etwa
...
\hyperlink{hyp:H1.1}{Hypothese H1.1}, wenngleich das absolute Limit hier durch die \ac{ALU}-Auslastung und nicht rein
durch den geometrischen Overdraw definiert wird.

\subsection{{Bandbreiten}}
\label{subsec:050302_bandwidth_eval}

% Einleitung.
Ziel der folgenden Evaluation ist die Isolierung der absoluten Speicherbandbreite (Fill-Rate) und des Basis-Overheads.
...
Im Rahmen dessen werden die Bandbreiten-Nachteile (Memory Tax) beim Schreiben und Lesen der Framebuffer
ausgelotet.
...
\begin{center}
    ...
    \colorbox{gray!5}{\parbox{0.95\textwidth}{
        \centering
        \vspace{2mm}
        {\sffamily\bfseries Versuchs-Setup: Base Bandwidth Tax (Dual-Auflösung)} \\[1.5ex]
        \begin{tabularx}{0.9\textwidth}{>{\sffamily}l X}
...
Auflösung von 4K im Vergleich zu anderen Single-Pass-Architekturen signifikant schlechter skaliert.

% Ergebnis Graph 1
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.9\linewidth]{Abbildungen/5-3-2-1_BaseBandwidthTax_Means}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.9\linewidth]{Abbildungen/5-3-2-2_BaseBandwidthTax_Means}
    \caption[Basis-Bandbreitenkosten der Rendering-Architekturen]{Vergleich der GPU-Ausführungszeit zwischen keiner
...
additiven Blendings (\textit{Read-Modify-Write}) zurückführen und liefert ein starkes Indiz für die
...
In Kombination mit dem simultanen Auslesen der breiten \ac{G-Buffer} (\ac{MRT}) resultiert dieser zusätzliche
...
Wie in Unterkapitel Kapitel~\ref{subsec:040301_forward_pipeline} beschrieben, sind sowohl die Forward-Pipeline
als auch die Deferred-Uber-Pipeline hinsichtlich der maximal nutzbaren Lichtquellen durch die
\ac{UBO}-Kapazität auf insgesamt 500 Punktlichter limitiert.
...
Darüber hinaus zeichnet sich das szenische Setup durch eine minimale Überlappung der Lichtquellen und deren Radien aus.
...
Deferred-Volume eine bemerkenswerte Effizienz und flache Skalierung aufweist.
...
skalieren als naive Forward-Ansätze.
...
Allgemeinen~\cites[S.~1]{Thaler2011DeferredRendering}.
Obwohl die Deferred-Uber-Pipeline den gleichen \ac{UBO}-Loop ausführt wie der Forward-Ansatz, wird durch das
Entkoppeln der Geometrie der Shading-Overdraw effektiv vermieden.
...
Komplexität der direkten Beleuchtungsberechnung zwingt die Forward-Architektur bei steigender Lichtdichte unweigerlich
...
\textit{Read-Modify-Write}-Bandbreitenlimits der Rasterisierungseinheiten.
...
darstellt.
Erhöht man den Radius-Multiplikator auf $20{,}0$ und steigert damit die Screen-Space-Abdeckung drastisch, so führt dies
...
erneut der Einfluss des Fragment-Overdraws untersucht.
In diesem Versuchsaufbau wird die Tiefenkomplexität allerdings unter Zuhilfenahme einer konstanten Beleuchtungslast
überprüft.
Der Aufbau ist hierbei nahezu identisch zu Unterkapitel~\ref{subsec:050203_overdraw_eval}, mit der Ausnahme von $100$
...
Bei der Betrachtung der Forward-Pipeline-Renderzeit ist ein Anstieg der kombinierten Zeit zu verzeichnen
(von \qty{1,11}{\milli\second} auf \qty{5,98}{\milli\second}, der jedoch nicht so katastrophal ausfällt, wie es die ursprüngliche
Annahme vermuten ließe.
...
% Deduktion
Die Ergebnisse dieser Evaluation widerlegen die weit verbreitete Annahme, dass der Fragment Overdraw, insbesondere in
Kombination mit Beleuchtung, in naiven Forward-Pipelines einen der größten Engpässe
...
verdeckter Fragmente vermieden~\cite[S.~801~u.~803]{akenine-mollerRealtimeRendering2019}.
...
aufgezeigt und somit zugleich die Fähigkeiten und Grenzen des HyDra-Frameworks demonstriert.
...
In der vorliegenden Untersuchung \textit{A} wird eine geklusterte (niedrige Entropie) mit einer gestreuten (hohe
...
und zu verifizieren.
Diese Analyse erfolgt insbesondere mit Blick auf die Performance der Deferred-Uber-Pipeline.
...
\begin{center}
    ...
    \colorbox{gray!5}{\parbox{0.95\textwidth}{
        ...
        {\sffamily\bfseries Versuchs-Setup} \\[1.5ex]
        \begin{tabularx}{0.9\textwidth}{>{\sffamily}l X}
            \multicolumn{2}{l}{\textbf{Test A: Räumliche Entropie}} \\
            Unabhängige Variable:    & Räumliche Verteilung ($E_{\text{Spatial}} \in \{\text{Geklustert},
            \text{Gestreut}\}$) \\
            Kontrollvariablen:& $N_{\text{Inst}} = 15.000$, $N_{\text{Light}} = 250$,
            $r_{\text{Spawn}} = 175{,}0$, Drei aktive Stile ($S_{\text{Count}} = 3$) \\[1.5ex]

            \multicolumn{2}{l}{\textbf{Test B: Kombinatorische Degradation}} \\
...
verdoppelt nahezu (Anstieg der Beleuchtungsberechnungs-Zeiten um $\approx$\,\qty{85}{\percent}) ihren Aufwand bei
einer gestreuten Stilverteilung (hohe Entropie).
...
\begin{figure}[htbp]
    ...
    der räumlichen Verteilung (geklustert vs.~gestreut) der Rendering-Stile.}
...
\begin{figure}[htbp]
    ...
    \begin{subfigure}[b]{0.49\textwidth}
        \centering
        \includegraphics[width=\textwidth]{Abbildungen/hydra_capture_20260628_110904}
        \caption{Geklusterte Verteilung}
    ...
    \begin{subfigure}[b]{0.49\textwidth}
        \centering
        \includegraphics[width=\textwidth]{Abbildungen/hydra_capture_20260628_110909}
        \caption{Gestreute Verteilung}
        \label{fig:hydra_random}
    \end{subfigure}
    \caption{Vergleich der Stil-Entropie Verteilungen in \textit{HyDra} während der 5.5.1 Test-Suite in Test A
        (Räumliche Entropie).}
...
% Deduktion.
In den theoretischen Grundlagen (vgl.\ Kapitel~\ref{subsec:020204_dynamic_branching_and_warp_divergence}) wurde
...
zusätzliche Hypothese eines erhöhten Register-Drucks aufstellen.
Die Implementierung eines zusätzlichen Rendering-Stils im monolithischen Code-Pfad bedingt potenziell die Allokation
weiterer \ac{VGPR} pro Thread.
Es kann deduziert werden, dass sich dadurch die globale Auslastung (Occupancy) der Recheneinheiten verringert
(vgl.~Kapitel~\ref{subsec:020203_register_pressure_limitations}).
...
Ohne den Einsatz dedizierter Hardware-Profiler lässt sich nicht zweifelsfrei differenzieren, inwiefern ein erhöhter
...
% Einleitung.
Der folgende Versuch fokussiert sich ausschließlich auf die Deferred-Pipelines (Deferred-Uber und \
textit{Deferred\_Volume}).
Die vorliegende Untersuchung widmet sich der Analyse der Einflüsse kernelbasierter \ac{NPR}-Effekte.
Aufgrund der Tatsache, dass solche kernelbasierten Effekte in der Forward-Pipeline auf Objektebene nicht realistisch
anwendbar sind (vgl.\ Kapitel~\ref{subsec:040301_forward_pipeline}) und nur auf gesamter Screen-Space-Ebene anwendbar
...
Wie bereits in Kapitel~\ref{subsec:040202_filter_operations} dargelegt, erfordern die beiden Architekturen
unterschiedliche Ansätze bei der Evaluierung des Kuwahara-Filters.
Die Deferred-Uber-Pipeline operiert als \textit{Pre-Lit}-Filter, um einen algorithmischen Ineffizienz-Kollaps durch
...
Kuwahara-Schleife einen zusätzlichen Texturlesezugriff auf den \textit{Light Accumulation Buffer}
(siehe algorithmische Beschreibung in Kapitel~\ref{subsec:040202_filter_operations}).

Über die Auswirkungen der asymmetrischen Implementationen des Kernel-Filters (Kuwahara) hinaus kann kein signifikanter
...
In Pass 2 (\textit{Shading Tax}) werden 250 dynamische Punktlichter mit aktivierter Umgebungsbeleuchtung hinzugefügt, um
...
Outlines) aktiviert, um die kombinierte Auswirkung von Beleuchtungs- und Entropielast zu evaluieren.
...
Das Deferred-Volume-Rendering erweist sich mit einer minimalen Erhöhung von lediglich $+\qty{0,03}{\milli\second}$ als
...
Sobald die Kamera in das Objekt- und Licht-Cluster eintaucht, können die jeweiligen Spitzen beobachtet werden.
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.9\linewidth]{Abbildungen/5_6_flythrough_variance_npr}
...
Die einfache Deferred-Uber-Architektur löst dieses Beleuchtungsproblem hocheffizient im Screen-Space,
...
Isolierung der Code-Pfade in Proxy-Geometrien, weist jedoch fundamentale Schwächen in der Speicherbandbreite auf.
